...
const seasons_url = 'https://ww5.gogoanimes.fi/sub-category/';
...
export const scrapeAnimeDetails = async ({ id }) => {
  try {
    ...
    const animePageTest = await axios.get(`https://ww5.gogoanimes.fi/category/${id}`);
    ...
    const desc = $('div.anime_info_body_bg > div.description > p').first().text();

    const releasedDate = $('div.anime_info_body_bg > p.type > span:contains("Released:")').parent().text().replace('Released: ', '');
    const status = $('div.anime_info_body_bg > p:nth-child(9) > a').text();
    const otherName = $('div.anime_info_body_bg > p.type.other-name > a').attr('title');
    ...
    return {
      ...
      imageUrl: "https://ww5.gogoanimes.fi"+ animeImage.toString(),
...
export const scrapeWatchAnime = async ({ id }) => {
  try {
    ...
    const WatchAnime = await axios.get(`https://ww5.gogoanimes.fi/${id}`);
    ...
    const gogoserver = $('li.doodstream > a').attr('data-video')
    const streamsb = $('li.streamwish > a').attr('data-video')
    const xstreamcdn = $('li.mp4upload > a').attr('data-video')
